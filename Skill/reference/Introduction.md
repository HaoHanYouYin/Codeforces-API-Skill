...
All methods can be requested anonymously. This way only public data will be accessable via API. To access data, private for some user (e.g. hacks during the contest), API key must be generated on [https://codeforces.com/settings/api](https://codeforces.com/settings/api) page. Each API key has two parameters: `key` and `secret`. To use the key you must add following parameters to your request.
